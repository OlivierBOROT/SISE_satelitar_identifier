...
\noindent\textbf{Remarque :} l'intégralité des 144 images a été exportée en un seul ensemble d'entraînement. Aucun découpage train/validation/test n'a été effectué lors de l'export Roboflow, ce qui constitue une limite méthodologique discutée en section~\ref{sec:discussion}.
...
Les hyperparamètres sont définis dans le fichier \texttt{hyp.scratch-low.yaml} et dans le code d'entraînement (notebook \texttt{02 - Training.ipynb}).
...
$\eta_{\min}$ (scheduler) & 0 & Valeur minimale du LR (défaut CosineAnnealingLR) \\
...
Nombre d'époques & 50 & Convergence observée (loss stabilisée) \\
Scheduler & CosineAnnealingLR & Décroissance douce du learning rate \\
$T_{\max}$ (scheduler) & 100 & Période complète du cosinus \\
...
\begin{itemize}
    ...
    \item \textbf{Sauvegarde} : les poids du modèle entraîné sont sauvegardés dans \texttt{yolov5n\_custom.pt}.
\end{itemize}
...
L'entraînement sur 50 époques montre une convergence régulière de la fonction de perte (figure~\ref{fig:loss}) :

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{loss_curve.png}
\caption{Évolution de la perte d'entraînement sur 50 époques.}
\label{fig:loss}
\end{figure}
...
\toprule
\textbf{Époque} & \textbf{Loss} & \textbf{Époque} & \textbf{Loss} & \textbf{Époque} & \textbf{Loss} \\
\midrule
1  & 6,4510 & 18 & 3,7376 & 35 & 2,3099 \\
5  & 4,7124 & 20 & 3,6782 & 40 & 1,9901 \\
10 & 4,5946 & 25 & 3,3791 & 45 & 1,7708 \\
15 & 3,9791 & 30 & 2,7162 & 50 & 1,6114 \\
\bottomrule
\end{tabular}
\caption{Évolution de la perte d'entraînement (moyennée par batch).}
\label{tab:loss}
\end{table}

La perte diminue de \textbf{6,45} à l'époque~1 à \textbf{1,61} à l'époque~50, soit une réduction de 75\,\%. La convergence est régulière sans signe de divergence, ce qui valide le choix des hyperparamètres. On observe une décroissance particulièrement rapide entre les époques~1 et~15 (phase d'adaptation des couches finales), puis une décroissance plus lente et stable jusqu'à l'époque~50 (fine-tuning des couches profondes).

\subsection{Résultats quantitatifs --- Métriques d'évaluation}

En l'absence de jeu de test séparé, nous évaluons le modèle sur l'ensemble d'entraînement lui-même (144 images) avec un seuil IoU de 0,5 et un seuil de confiance de 0,25. Le tableau~\ref{tab:metrics} résume les performances par classe. \textbf{Ces métriques reflètent donc la capacité du modèle à restituer les données vues pendant l'entraînement et non sa capacité de généralisation}, ce qui constitue une limite importante de cette évaluation.

\begin{table}[H]
\centering
\begin{tabular}{lcccccc}
\toprule
\textbf{Classe} & \textbf{TP} & \textbf{FP} & \textbf{FN} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\midrule
\texttt{ferme}    & 56  & 35  & 93  & 0,615 & 0,376 & 0,467 \\
\texttt{immeuble} & 270 & 143 & 518 & 0,654 & 0,343 & 0,450 \\
\texttt{maison}   & 803 & 318 & 442 & 0,716 & 0,645 & 0,679 \\
\texttt{piscine}  & 144 & 171 & 49  & 0,457 & 0,746 & 0,567 \\
\texttt{usine}    & 19  & 22  & 50  & 0,463 & 0,275 & 0,345 \\
\texttt{villa}    & 9   & 2   & 8   & 0,818 & 0,529 & 0,643 \\
\midrule
\textbf{Total}    & 1\,301 & 691 & 1\,160 & 0,653 & 0,529 & 0,584 \\
\textbf{mAP@0.5}  & \multicolumn{6}{c}{\textbf{0,621}} \\
\bottomrule
\end{tabular}
\caption{Precision, Recall et F1-score par classe (IoU $\geq$ 0,5, confiance $\geq$ 0,25).}
\label{tab:metrics}
\end{table}

Le modèle atteint un \textbf{mAP@0.5 de 0,621} et un \textbf{F1-score global de 0,584}. La classe \texttt{maison}, la plus représentée, obtient les meilleurs résultats (F1~=~0,679). Notons le cas intéressant de \texttt{villa} : malgré seulement 17 annotations dans le dataset, le modèle atteint une precision de 0,818, suggérant que les quelques exemples disponibles sont visuellement distinctifs. À l'inverse, \texttt{usine} (F1~=~0,345) et \texttt{immeuble} en recall (0,343) sont les points faibles, ce dernier souffrant d'un grand nombre de faux négatifs (518).
...
L'inférence est réalisée dans le notebook \texttt{03 - Testing\_model.ipynb} avec les paramètres suivants :
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{Redimensionnement} avec \emph{letterboxing} à $640 \times 640$ (padding gris 114).
...
\begin{itemize}
    ...
    \item \textbf{Absence de jeu de validation structuré} dans la boucle d'entraînement : la perte de validation n'est pas suivie explicitement, ce qui empêche de détecter un éventuel sur-apprentissage.
    \item \textbf{Déséquilibre des classes} : avec 1\,245 annotations pour \texttt{maison} contre seulement 17 pour \texttt{villa} et 69 pour \texttt{usine}, le modèle est biaisé vers les classes majoritaires. Des techniques de rééquilibrage (sur-échantillonnage, perte focale) pourraient améliorer les performances sur les classes rares.
\end{itemize}

\subsection{Perspectives d'amélioration}

\begin{enumerate}
    \item \textbf{Augmenter le volume de données} : le pipeline de collecte peut facilement être étendu à d'autres coordonnées ou pays.
    \item \textbf{Ajouter un suivi de validation} : calcul du mAP@0.5 et mAP@0.5:0.95 sur un jeu de validation à chaque époque.
    \item \textbf{Entraîner plus longtemps} : la perte continue de décroître à l'époque~50, suggérant qu'un entraînement plus long (100--200 époques) pourrait améliorer les performances.
    \item \textbf{Augmentation de données avancée} : activer \texttt{mixup}, \texttt{copy\_paste}, ou utiliser des rotations pour mieux couvrir la variabilité géographique.
    \item \textbf{Modèle plus grand} : tester YOLOv5s ou YOLOv5m si le jeu de données est élargi.
    \item \textbf{Segmentation} : passer à YOLOv5-seg ou à un modèle de segmentation d'instance pour des contours plus précis.
\end{enumerate}
...
L'architecture YOLOv5 Nano, choisie pour sa légèreté et son efficacité, a montré une convergence satisfaisante lors de l'entraînement (loss passant de 6,45 à 1,61 en 50 époques). L'évaluation quantitative révèle un mAP@0.5 de \textbf{0,621} et un F1-score global de \textbf{0,584}, des résultats encourageants étant donné la taille réduite du dataset (144 images) et le fort déséquilibre entre classes. Le pipeline développé --- de la collecte Mapbox/OSM à l'inférence --- est entièrement reproductible et extensible, constituant une base solide pour de futures améliorations.
